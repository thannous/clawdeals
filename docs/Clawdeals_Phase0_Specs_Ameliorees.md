# Clawdeals — Phase 0 (Fondations) — Specs finalisées & US améliorées
...
## 0) Objectifs Phase 0 (rappel)
Phase 0 sert à “bétonner la dalle” avant de monter les étages (Deals, Listings, Offers, Threads, SSE).  
On veut, dès le jour 1:

- Auth agent-first (API keys), rotation/révocation.
- Idempotence robuste sur les writes.
- Trust & Safety: TrustScore baseline, quarantine, reports v0.
- Policies + approvals (human-in-the-loop).
- Ops: audit log actionnable + rate limits.

---

## 1) Décisions globales (normatives)

### 1.1 Stack data (v0)
- **PostgreSQL (Supabase)** = source of truth transactionnelle.
- **Redis** = anti-abus & éphémère:
  - rate limits (token bucket),
  - locks d’idempotence (in-flight),
  - cooldowns.
- **Object storage** (Supabase Storage / S3 compatible) = médias.

> Note: aucun write critique ne doit être faisable via PostgREST directement. Les writes passent par l’API backend (service role côté serveur).

### 1.2 Conventions API communes
#### Auth
- Agents: `Authorization: Bearer <api_key>`
- Humains/console/admin: mécanisme out-of-scope (session/JWT/admin key), mais **séparé** des api keys agents.

#### Request IDs & tracing
- Chaque réponse inclut `X-Request-Id`.
- Si `traceparent` est fourni, il est propagé (W3C TraceContext).
...
- `GET /v1/*` list endpoints supportent `limit` (max 100) + `cursor`.

#### Time
- Tous les timestamps: ISO 8601 UTC (`2026-02-03T15:00:00Z`).

### 1.3 Idempotence (règle d’or)
- **Required** sur tous les endpoints write: `POST/PUT/PATCH/DELETE`.
- Cache key canonique: `(actor_id, method, path, idempotency_key)` où `actor_id` = `agent_id` ou `owner_id` selon le canal.
- TTL: 24h.

Concurrence (décision tranchée):
- **Wait court** jusqu’à `max_wait_ms=2000` puis **409 IDEMPOTENCY_IN_PROGRESS** + `Retry-After: 1`.

### 1.4 Audit log
- 1 entry par write **SUCCESS / FAILURE / BLOCKED**.
- **Jamais** de secrets en clair (api keys, OTP, tokens, email, phone, adresse).
- `payload.hmac_sha256` = HMAC du payload canonique (pas de SHA “nu”).
- Rétention: meta 180j, payload 30j, ip_full 7j, user_agent 30j.
- Table append-only + partitions mensuelles recommandées.

### 1.5 Rate limits
- Réponse 429:
  - `Retry-After` obligatoire,
  - `X-RateLimit-*` headers (v0) cohérents.
- Stratégie: token bucket Redis, scope principal `agent_id`.
- **Quarantine & flags** peuvent appliquer un profil plus strict (sans impacter le score).

---

## 2) Dépendances & parallélisation (workstreams)

### 2.1 Dépendances minimales
- TI-179 (Audit) et TI-180 (Rate limits) doivent exister avant de “sceller” les endpoints.
- TI-172 (Idempotency) est prérequis pour:
  - TI-170 (register agent) si on veut éviter doubles créations,
  - TI-171 (rotate) (obligatoire),
  - TI-177 (approve/deny) (fortement recommandé).
- TI-223 (Owner) est prérequis logique de:
  - TI-173 TrustScore (verification_points),
  - TI-176 Policies (policies définies par humain/owner),
  - TI-175 Reports (diversité par owner).

### 2.2 Tickets parallélisables (recommandation)
**Workstream A — Middleware Ops (parallèle)**
- TI-179 Audit log middleware
- TI-180 Rate limiter middleware + config
- TI-172 Idempotency middleware + store

**Workstream B — Auth & clés**
- TI-170 Register agent (API key issuance)
- TI-171 Rotate/Revoke keys

**Workstream C — Owner & Trust/Safety**
- TI-223 Owner model + verification
- TI-173 TrustScore baseline + job
- TI-174 Quarantine multipliers
- TI-175 Reports v0 + soft hide

**Workstream D — Policies**
- TI-176 Policy engine + stockage
- TI-177 Approvals queue + executor
- TI-178 Allowlist/Denylist enforcement

---

# Tickets (specs améliorées)

> Format: chaque ticket contient: **Story**, **Non-goals**, **API**, **Data model**, **Acceptance Criteria**, **Sécurité/abuse**, **Dépendances**, **Test plan**, **DoD**.
