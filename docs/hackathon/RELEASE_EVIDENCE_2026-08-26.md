...
| CI current HEAD | WAIVED / NOT RUN on `60b99f7` | The owner explicitly waived fresh remote GitHub Actions runs and asked to ignore the zero-dollar Actions budget. This row is neither PASS nor FAIL. |
| DEPLOYED | PASS on production hub for `60b99f7` | After the push on 29 August, the Vercel-backed public hub rendered the full commit `60b99f70868fc70a2b947a8a70c4e2212e174f3a`. This is production-hub deployment proof, not the isolated authenticated sandbox. |
| PUBLIC HTTP | PASS on `60b99f7` | `https://clawdeals.com/webmcp-challenge` returned 200 with `Origin-Agent-Cluster: ?1`; public listings returned 200; production `GET /api/v1/sandbox/reset` remained 404. Native WebMCP, the sandbox journey and database migration state remain separate. |
...
| REMOTE sandbox infrastructure | PARTIAL, advanced on 29 August | The isolated Supabase branch has the four final challenge migrations and service-role-only execute grants. Dedicated Upstash Redis, masked staging secrets, Vercel custom domain, public DNS-only CNAME and TLS are provisioned. The sandbox host currently returns the expected Vercel 404 because Git connection, deployment and public runtime proof remain pending. |
| PUBLIC authenticated journey | PENDING | The eleven-tool authenticated registry, sandbox reset and critical mutation path have not been proved on a final public sandbox host. Infrastructure is ready through DNS, but Vercel has no deployment until the separately confirmed persistent Git connection; see [`PUBLIC_SANDBOX_PLAN_2026-08-26.md`](./PUBLIC_SANDBOX_PLAN_2026-08-26.md). Production stays non-sandbox. |
