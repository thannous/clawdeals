...
| DEPLOYED | PASS for runtime ancestry through `fc29e66` | On 27 August the Vercel-backed public hub displayed documentation descendant `e1b46c99210b`, which contains runtime `fc29e66` in its ancestry. This does not prove database migrations were applied. |
| PUBLIC HTTP | PASS on descendant `e1b46c9` | `https://clawdeals.com/webmcp-challenge` returned HTTP 200 with `Origin-Agent-Cluster: ?1`, and production `GET /api/v1/sandbox/reset` returned 404. Native WebMCP and database migration state remain separate. |
...
| REMOTE sandbox infrastructure | PARTIAL, advanced on 29 August | The isolated Supabase branch has the four final challenge migrations and service-role-only execute grants. Dedicated Upstash Redis, masked staging secrets, Vercel custom domain and the public DNS-only CNAME are provisioned. Git connection, deployment and public runtime proof remain pending. |
