...
suite("GET /v1/transactions/{tx_id} (TI-203)", () => {
  ...
  let getContactsForTransactionMock: any;
  ...
  beforeAll(async () => {
    ...
    vi.doMock("../../../../server/services/transactions", () => ({
      ...
      getContactsForTransaction: vi.fn(),
    ...
    getContactsForTransactionMock = vi.mocked(txMod.getContactsForTransaction);
  }, 30_000);
  ...
  beforeEach(() => {
    ...
    getContactsForTransactionMock.mockResolvedValue({
      buyer_contact: { email_masked: "b***@e******.com", phone_masked: "+33 ** ** ** 12 34" },
      seller_contact: {
        email_masked: "s***@g****.fr",
        phone_masked: "+33 ** ** ** 56 78",
        email: "seller@gmail.fr",
        phone: "+33611225678"
      }
    } as any);
  });
  ...
  it("includes contacts revealed to the calling party when APPROVED", async () => {
    getTransactionMock.mockResolvedValue({
      tx_id: txId,
      listing_id: listingId,
      thread_id: "33333333-3333-4333-8333-333333333333",
    ...
    expect(result.status).toBe(200);
    expect(getContactsForTransactionMock).toHaveBeenCalledWith(expect.anything(), {
      revealToAgentId: buyerAgentId
    });
    expect(result.body.data?.buyer_contact?.email_masked).toContain("***");
    expect(result.body.data?.seller_contact?.email).toBe("seller@gmail.fr");
  });

  it("keeps contacts masked for the ops console when APPROVED", async () => {
    getTransactionMock.mockResolvedValue({
      tx_id: txId,
      listing_id: listingId,
      thread_id: "33333333-3333-4333-8333-333333333333",
