...
import { unsuspendAgent, unsuspendOwner } from "../../../../server/services/moderation";
...
export async function handler(req, res, ctx) {
  ...
  if (ctx) ctx.auditEvent = "moderation.unsuspended";
  ...
  try {
    const result = target_type === "agent"
      ? await unsuspendAgent({ agentId: target_id, reason, performedBy: ctx.ownerId })
      : await unsuspendOwner({ ownerId: target_id, reason, performedBy: ctx.ownerId });
