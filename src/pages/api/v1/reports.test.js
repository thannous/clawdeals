...
function baseCtx() {
  return { agentId, ownerId, authError: null };
}
...
describe("POST /v1/reports", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resolveTrustContext.mockResolvedValue({ trust_score: 50, trust_flags: [], quarantine_applied: false });
  });
  ...
  it("returns 401 on authError", async () => {
    ...
    const ctx = { authError: { status: 401, code: "UNAUTHORIZED", message: "Invalid" } };
  ...
  it("returns 401 without agentId", async () => {
    const req = makeReq(validBody());
    const ctx = { agentId: null, ownerId, authError: null };
    const result = await handler(req, null, ctx);
    expect(result.status).toBe(401);
  });

  it("returns 400 for invalid entity_type", async () => {
    const result = await handler(makeReq(validBody({ entity_type: "invalid" })), null, baseCtx());
    expect(result.status).toBe(400);
    expect(result.body.error.message).toContain("entity_type");
  });

  it("returns 400 for non-UUID entity_id", async () => {
    const result = await handler(makeReq(validBody({ entity_id: "not-uuid" })), null, baseCtx());
    expect(result.status).toBe(400);
    expect(result.body.error.message).toContain("entity_id");
  });

  it("returns 400 for invalid reason_code", async () => {
    const result = await handler(makeReq(validBody({ reason_code: "invalid" })), null, baseCtx());
    expect(result.status).toBe(400);
    expect(result.body.error.message).toContain("reason_code");
  });

  it("returns 400 for free_text over 500 chars", async () => {
    const result = await handler(makeReq(validBody({ free_text: "a".repeat(501) })), null, baseCtx());
    expect(result.status).toBe(400);
    expect(result.body.error.message).toContain("500");
  });

  it("returns 400 without reporter owner_id", async () => {
    const ctx = { agentId, ownerId: null, authError: null };
    const result = await handler(makeReq(validBody()), null, ctx);
    expect(result.status).toBe(400);
  ...
  it("returns 409 on duplicate report", async () => {
    ...
    const result = await handler(makeReq(validBody()), null, baseCtx());
