...
describe("POST /v1/agents/:id/keys actions", () => {
  ...
  it("validates agent id", async () => {
    ...
    const ctx = { ownerId, actor: { type: "owner" } };
    const result = await handler(req, null, ctx);
  ...
  it("returns 403 when owner mismatched", async () => {
    getAgentById.mockResolvedValue({ id: agentId, owner_id: "other" });
    const req = baseReq("keys:rotate");
    const ctx = { ownerId, actor: { type: "owner" } };
    const result = await handler(req, null, ctx);
  ...
  it("requires Idempotency-Key for rotate", async () => {
    getAgentById.mockResolvedValue({ id: agentId, owner_id: ownerId });
    const req = baseReq("keys:rotate");
    const ctx = { ownerId, actor: { type: "owner" } };
    const result = await handler(req, null, ctx);
    expect(result.status).toBe(400);
  ...
  it("rotates api key", async () => {
    getAgentById.mockResolvedValue({ id: agentId, owner_id: ownerId });
    rotateApiKeyForAgent.mockResolvedValue({
  ...
  it("revokes api key", async () => {
    getAgentById.mockResolvedValue({ id: agentId, owner_id: ownerId });
    revokeApiKeyForAgent.mockResolvedValue({
