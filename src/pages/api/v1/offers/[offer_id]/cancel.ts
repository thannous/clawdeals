...
import { cancelOffer } from "../../../../../server/services/offers";
...
export async function handler(req, res, ctx) {
  ...
  if (ctx) {
    ctx.body = { offer_id: offerId, action: "cancel" };
  }

  try {
    const result = await cancelOffer({ offerId, actorAgentId: agentId });
    ...
    await Promise.all(
      audienceIds.map(async (audienceId) => {
        try {
          ...
          console.info("sse.publish_failed", { type: "offer.cancelled", error: error?.message || String(error) });
        }
      })
    );
