...
export default function WebMcpChallengePage() {
  ...
  const [resetState, setResetState] = useState<"idle" | "running" | "done" | "failed">("idle");
  const [resetResult, setResetResult] = useState<ResetResult | null>(null);

  useEffect(() => {
    const controller = new AbortController();
    setCapability({ loading: true, enabled: false, authorized: false, error: null });
      ...
      <main id="main-content" tabIndex={-1} className="pb-20 pt-24">
        <section className="mx-auto grid max-w-[1440px] gap-8 px-4 sm:px-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)] lg:items-end">
          <div>
            ...
            <p className="mt-6 max-w-3xl text-base leading-relaxed text-muted sm:text-lg">
              ClawDeals is the trust layer for delegated commerce: agents search, rank and negotiate;
              owners set the limits; the platform enforces approvals, bilateral consent and verifiable receipts.
            </p>
        ...
        <section className="mx-auto mt-16 grid max-w-[1440px] gap-6 px-4 sm:px-6 lg:grid-cols-[minmax(0,.8fr)_minmax(0,1.2fr)]">
          <div className="border border-border bg-surface p-5 sm:p-6">
            ...
            <button
              type="button"
              data-testid="webmcp-challenge-reset"
              disabled={!capability.authorized || resetState === "running"}
              onClick={resetDemo}
              className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 border border-primary px-4 font-mono text-xs font-bold uppercase tracking-widest text-primary transition enabled:hover:bg-primary enabled:hover:text-bg disabled:cursor-not-allowed disabled:border-border disabled:text-subtle"
            >
              <RotateCcw className={`h-4 w-4 ${resetState === "running" ? "animate-spin" : ""}`} aria-hidden="true" />
              {resetLabel}
            </button>
            <div className="mt-3 min-h-10 font-mono text-[11px] leading-relaxed text-subtle" data-testid="webmcp-challenge-reset-status">
              {capability.loading
              ...
              {resetState === "done" && resetResult?.counts ? (
                <span className="mt-2 block text-success" data-testid="webmcp-challenge-reset-result">
                  Ready: {resetResult.counts.listings || 0} listings · {resetResult.counts.threads || 0} thread · {resetResult.counts.messages || 0} message.
                </span>
              ) : null}
              {resetState === "failed" ? <span className="mt-2 block text-error">Reset rejected. Verify the isolated host and judge key.</span> : null}
            </div>
          </div>
