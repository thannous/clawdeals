...
describe("resetSandboxFixtures", () => {
  ...
  it("allows fixture seeding against local and staging Supabase targets", async () => {
    ...
    await expect(resetSandboxFixtures({ agentId, now })).resolves.toMatchObject({ ok: true });

    process.env.SUPABASE_URL = LOCAL_SUPABASE_URL;
    process.env.NEXT_PUBLIC_SUPABASE_URL = LOCAL_SUPABASE_URL;
    await expect(resetSandboxFixtures({ agentId, now })).resolves.toMatchObject({ ok: true });
    expect(getSupabaseServiceClient).toHaveBeenCalledTimes(2);
  });
