-- TI-314: Fix revoke_installation_v1 ambiguous column references.
--
-- In plpgsql, the function's output column names become variables in scope.
-- Unqualified SQL references like `where installation_id = ...` can become
-- ambiguous between the output variable and a table column. Qualify columns
-- with table aliases to fail closed and avoid runtime 500s.
