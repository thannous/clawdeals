-- Fix resolve_approval(): avoid PL/pgSQL variable conflicts with column names (thread_id, listing_id, etc.)
-- This happened after TI-196/TI-197 migration introduced `threads.thread_id` + `messages.thread_id`.
...
begin
  ...
  if approval_row.action_type = 'thread.create' then
    ...
    if v_message_payload is not null and v_message_payload <> '{}'::jsonb then
      ...
      v_sender_type := case when v_buyer_agent_id is not null then 'agent' else 'human' end;
    ...
    v_sender_type := case when v_agent_id is not null then 'agent' else 'human' end;
    v_message_type := nullif(v_message_payload->>'type', '');
