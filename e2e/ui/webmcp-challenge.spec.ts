...
test.describe("WebMCP Challenge judge entry", () => {
  ...
  test("shows the real public registry, eligibility evidence, live demo, and copyable mission", async ({ page }) => {
    ...
    const registered = await page
      .getByTestId("webmcp-challenge-registered-tool")
      .allTextContents();
    ...
    await expect(page.getByRole("link", { name: "Full eligibility ledger" })).toHaveAttribute(
      "href",
      /HACKATHON\.md$/
    );
    await expect(page.getByRole("link", { name: "WebMCP evals" })).toHaveAttribute(
      "href",
      /evals\/webmcp$/
    );
  ...
  test("rebuilds the same judge fixture twice and clears stale local receipts", async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      window.localStorage.setItem(
        ...
        JSON.stringify([{ receipt_version: "1", receipt_id: "stale", request_id: "stale", tool: { name: "stale" }, timestamp: new Date().toISOString() }])
      );
      Object.defineProperty(document as any, "modelContext", {
        configurable: true,
    ...
    expect(await page.getByTestId("webmcp-challenge-registered-tool").allTextContents()).toEqual(
      AUTHENTICATED_TOOLS
    );
    await expect(page.getByTestId("webmcp-challenge-reset")).toBeEnabled();

    await page
      .getByTestId("webmcp-challenge-reset")
